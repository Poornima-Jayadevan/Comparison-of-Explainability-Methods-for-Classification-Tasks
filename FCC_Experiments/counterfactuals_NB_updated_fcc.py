...
class EncodedCategoricalNBWrapper:

    # Wrap the saved encoder + CategoricalNB model so DiCE can call it using original categorical feature values.

    def __init__(self, model, feature_encoder, label_encoder):
        self.model = model
    ...
    def predict_proba(self, samples) -> np.ndarray:
        ...
        encoded = self.feature_encoder.transform(frame).astype(int)
...
def load_artifacts():
    """Load data, final model, and encoders."""
    for file_path in [
        DATA_FILE,
        MODEL_FILE,
    ...
    wrapper = EncodedCategoricalNBWrapper(
        model,
        feature_encoder,
        label_encoder,
    )

    return df, wrapper, label_encoder



def class_name(label_encoder, encoded_class: int) -> str:
    """Convert a numeric class ID to Keep/Discard."""
    return str(
        label_encoder.inverse_transform(
            np.asarray([int(encoded_class)])
        )[0]
    )


def class_index(label_encoder, class_label: str) -> int:
    """Convert Keep/Discard to its numeric class ID."""
...
def select_representative_indices(
    ...
    """Select the same case types used in SHAP and LIME."""
    predicted_encoded = wrapper.predict(df[FEATURE_COLUMNS])
    predicted_labels = label_encoder.inverse_transform(
        predicted_encoded.astype(int)
    )
    actual_labels = df[TARGET_COLUMN].to_numpy()
...
def opposite_class(predicted_label: str) -> str:
    """Return the opposite Keep/Discard class."""
    return "Discard" if predicted_label == "Keep" else "Keep"
...
def build_dice_objects(
    ...
    """Create DiCE data and model interfaces using numeric class labels."""
    dice_data_frame = df[
        FEATURE_COLUMNS + [TARGET_COLUMN]
    ].copy()

    dice_data_frame[TARGET_COLUMN] = label_encoder.transform(
    ...
    return data_interface, model_interface, explainer


def extract_counterfactual_dataframe(
    explanation,
) -> pd.DataFrame:
    """Extract the generated counterfactual DataFrame from DiCE output."""
    ...
    candidate_frames = [
        getattr(example, "final_cfs_df", None),
        getattr(example, "final_cfs_df_sparse", None),
    ]
...
def categorical_hamming_ratio(
    ...
    """Proportion of categorical feature states that differ."""
    changes = [
        first_row[feature] != second_row[feature]
...
def changed_features(
    ...
    """Return the names of features changed in the counterfactual."""
    return [
        feature
        for feature in FEATURE_COLUMNS
...
def mean_pairwise_diversity(
    ...
    """Mean pairwise Hamming distance among generated counterfactuals."""
    ...
    for first_index, second_index in combinations(
        ...
        first = counterfactuals.iloc[first_index]
        second = counterfactuals.iloc[second_index]

        distances.append(
            categorical_hamming_ratio(first, second)
        )

    return float(np.mean(distances)) if distances else 0.0


def readable_state(feature_name: str, value: str) -> str:
    """Create readable labels for plots."""
    labels = {
        "chain_length_bin": "Length",
        "direction_change_bin": "Direction change",
        "fuzziness_bin": "Fuzziness",
    }
    return f"{labels[feature_name]} = {value}"
...
def save_counterfactual_plot(
    ...
    """Save a before-and-after categorical state plot."""
    original_labels = [
        readable_state(feature, original[feature])
        for feature in FEATURE_COLUMNS
    ]
    counterfactual_labels = [
        readable_state(feature, counterfactual[feature])
        for feature in FEATURE_COLUMNS
    ]

    rows = np.arange(len(FEATURE_COLUMNS))
    ...
    for row_index, (original_text, cf_text) in enumerate(
        zip(original_labels, counterfactual_labels)
    ):
        ax.text(
            -0.03,
            row_index,
    ...
    ax.set_xlim(-0.8, 1.8)
    ax.set_xticks([0, 1], ["Original", "Counterfactual"])
    ax.set_yticks([])
    ax.set_title(
        f"{title}\n"
        f"Original P(Keep) = {original_probability_keep:.4f}, "
...
def categorical_change_vector(
    ...
    1 = feature state changed
    0 = feature state unchanged
    """
    return np.asarray(
        [
...
def main() -> None:
    df, wrapper, label_encoder = load_artifacts()
    _, _, explainer = build_dice_objects(df, wrapper, label_encoder)
    ...
    output_names = {
        "keep": "counterfactual_local_keep.png",
        "discard": "counterfactual_local_discard.png",
        "misclassified": "counterfactual_local_misclassified.png",
    }
    ...
    print("=" * 72)
    print("DiCE Counterfactual Analysis — Categorical Naive Bayes")
    print("=" * 72)
    print(f"Features: {FEATURE_COLUMNS}")
    print(
        f"Counterfactuals requested per representative fringe: "
    ...
    for case_type, sample_index in selections.items():
        ...
        predicted_class = int(
            wrapper.predict(query_instance)[0]
        )
        predicted_label = class_name(
            label_encoder,
            predicted_class,
        )
        desired_label = opposite_class(predicted_label)
        desired_class = class_index(
            label_encoder,
            desired_label,
        )
        actual_label = str(original_row[TARGET_COLUMN])

        original_probabilities = wrapper.predict_proba(
            query_instance
        )[0]
        keep_index = class_index(label_encoder, "Keep")
        original_probability_keep = float(
            original_probabilities[keep_index]
        )
        ...
        counterfactuals = extract_counterfactual_dataframe(
            explanation
        )
            ...
            metric_rows.append(
                {
                    ...
                    "counterfactuals_requested": TOTAL_COUNTERFACTUALS,
                    "counterfactuals_generated": 0,
                    "validity": 0.0,
                    "mean_changed_feature_count": np.nan,
                    "mean_changed_feature_ratio": np.nan,
                    "mean_proximity_hamming": np.nan,
        ...
        if TARGET_COLUMN not in counterfactuals.columns:
            counterfactuals[TARGET_COLUMN] = wrapper.predict(
                counterfactuals[FEATURE_COLUMNS]
            ).astype(int)

        predicted_cf_classes = wrapper.predict(
            counterfactuals[FEATURE_COLUMNS]
        ).astype(int)
        predicted_cf_labels = label_encoder.inverse_transform(
            predicted_cf_classes
        )
        counterfactuals["verified_prediction"] = predicted_cf_labels

        cf_probabilities = wrapper.predict_proba(
            counterfactuals[FEATURE_COLUMNS]
        )
        counterfactuals["probability_keep"] = (
            cf_probabilities[:, keep_index]
        )

        valid_mask = (
            counterfactuals["verified_prediction"]
            == desired_label
        )
        ...
        for cf_index, cf_row in counterfactuals.iterrows():
            changed = changed_features(
                original_row,
                cf_row,
            )
            changed_count = len(changed)
            changed_ratio = changed_count / len(FEATURE_COLUMNS)
            sparsity_score = counterfactual_sparsity_score(
                changed_count
            )
            proximity = categorical_hamming_ratio(
                original_row,
                cf_row,
            )
            ...
            explanation_row = {
                "case_type": case_type,
                "fringe_id": fringe_id,
                "counterfactual_id": cf_index + 1,
                "actual_label": actual_label,
                "original_prediction": predicted_label,
                "desired_class": desired_label,
                "counterfactual_prediction": str(
                    cf_row["verified_prediction"]
                ),
                ...
                "counterfactual_probability_keep": float(
                    cf_row["probability_keep"]
                ),
                "prediction_probability_shift": float(
                    abs(
                        original_probability_keep
                        - float(cf_row["probability_keep"])
                    )
                ),
                "changed_feature_count": changed_count,
                "changed_feature_ratio": changed_ratio,
                "sparsity_score": sparsity_score,
                "proximity_hamming": proximity,
                "changed_features": ", ".join(changed),
            }

            for feature in FEATURE_COLUMNS:
                explanation_row[f"original_{feature}"] = (
                    original_row[feature]
                )
                explanation_row[f"counterfactual_{feature}"] = (
                    cf_row[feature]
                )

            explanation_rows.append(explanation_row)

        diversity = mean_pairwise_diversity(
            counterfactuals
        )
        ...
        metric_rows.append(
            {
                ...
                "validity": validity,
                "mean_changed_feature_count": float(
                    np.mean(changed_counts)
                ),
                "mean_changed_feature_ratio": float(
                    np.mean(changed_ratios)
                ),
                "mean_sparsity_score": float(
                    np.mean(sparsity_scores)
                ),
                "mean_proximity_hamming": float(
                    np.mean(proximity_values)
                ),
        ...
        # Plot the first valid counterfactual, otherwise the first generated one.
        ...
        selected_cf = counterfactuals.iloc[selected_cf_index]

        title = (
            f"DiCE Counterfactual — Fringe {fringe_id}\n"
        ...
        save_counterfactual_plot(
            ...
            title=title,
            original=original_row,
            counterfactual=selected_cf,
            original_probability_keep=original_probability_keep,
            counterfactual_probability_keep=float(
                selected_cf["probability_keep"]
            ),
        )
    ...
    evaluation_filename = "nb_counterfactual_evaluation_metrics.csv"
    ...
    print("\nSaved files:")
    print("nb_counterfactual_evaluation_metrics.csv")
    print("counterfactual_explanations.csv")
    print("counterfactual_metrics.csv")
