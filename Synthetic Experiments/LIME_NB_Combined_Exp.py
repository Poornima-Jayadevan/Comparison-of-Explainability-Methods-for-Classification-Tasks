...
# =====================================
# Load saved NB model & training data
# =====================================
nb = joblib.load("nb_combined_model.pkl")
X_train = joblib.load("X_train_combined_nb.pkl")
X_test = joblib.load("X_test_combined_nb.pkl")
y_test = joblib.load("y_test_combined_nb.pkl")
feature_names = joblib.load("feature_names_combined_nb.pkl")

# Load dataset for instance selection & visualization
...
print("\nModel predictions:")
print(
    "Clean prediction:", nb.predict(clean_instance.reshape(1, -1))[0],
    "Proba:", nb.predict_proba(clean_instance.reshape(1, -1))[0]
)
print(
    "Boundary prediction:", nb.predict(boundary_instance.reshape(1, -1))[0],
    "Proba:", nb.predict_proba(boundary_instance.reshape(1, -1))[0]
)
print(
    "Outlier prediction:", nb.predict(outlier_instance.reshape(1, -1))[0],
    "Proba:", nb.predict_proba(outlier_instance.reshape(1, -1))[0]
)
...
plt.suptitle("LIME Explanation Comparison: Clean vs Boundary vs Outlier (NB)")
plt.tight_layout()
plt.show()
plt.close()
...
# =====================================
# Grouped LIME weight comparison bar chart
# =====================================
plot_df = comparison_df.set_index("Feature")
plot_df.plot(kind="bar", figsize=(8, 5))
plt.title("LIME Weight Comparison: Clean vs Boundary vs Outlier (NB)")
...
plt.axhline(0, color="black", linewidth=1)
plt.ylabel("LIME Weight")
plt.title("LIME Stability Across Repeated Runs (NB Combined Dataset)")
plt.tight_layout()
plt.show()
plt.close()
...
plt.xlabel("Run")
plt.ylabel("LIME Weight")
plt.title("LIME Weight Variation Across Repeated Runs (NB Combined Dataset)")
plt.legend()
plt.tight_layout()
plt.show()
plt.close()
...
# =====================================
# LIME Fidelity - Clean vs Boundary vs Outlier 
# =====================================
def compute_lime_fidelity(instance, exp_obj, model):
    ...
    lime_local_pred = exp_obj.local_pred[0]  # probability predicted by LIME
...
# Compute fidelity for the 3 key instances
...
fidelity_outlier = compute_lime_fidelity(outlier_instance, exp_outlier, nb)
...
fidelity_df = pd.DataFrame({
    ...
    "Predicted Class (NB)": [fidelity_clean["predicted_class"], fidelity_boundary["predicted_class"], fidelity_outlier["predicted_class"]],
    "NB Pred Prob": [fidelity_clean["nb_pred_prob"], fidelity_boundary["nb_pred_prob"], fidelity_outlier["nb_pred_prob"]],
...
print("\nLIME Fidelity Comparison (NB Combined Dataset):")
print(fidelity_df)

# =====================================
# Bar chart for Fidelity Error vs LIME exp.score
# =====================================
...
ax.set_title("LIME Fidelity Comparison: NB Combined Dataset")
ax.set_xticks(x)
...
print("\nLIME Stability Comparison - NB Combined Dataset:")
...
# Compute sparsity for all three explanations
...
outlier_weight_vector, outlier_non_zero, outlier_total, outlier_sparsity = compute_lime_sparsity(
    weights_outlier,
    feature_names
)
...
sparsity_df = pd.DataFrame({
    ...
    "Weight Vector": [
        clean_weight_vector,
        boundary_weight_vector,
        outlier_weight_vector
    ],
    "Non-zero Features": [
        clean_non_zero,
        boundary_non_zero,
...
plt.xlabel("Instance")
plt.title("LIME Sparsity - NB Combined Dataset")
plt.axhline(0, color="black", linewidth=1)
